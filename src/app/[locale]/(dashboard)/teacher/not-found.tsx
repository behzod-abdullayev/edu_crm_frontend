...
/**
 * src/app/[locale]/(dashboard)/teacher/not-found.tsx
 *
 * Teacher-section 404 page — Client Component.
 *
 * Rendered by Next.js when `notFound()` is called inside any Server Component
 * under src/app/[locale]/(dashboard)/teacher/**, OR when a user navigates to
 * a URL that starts with /[locale]/teacher/... but does not match any route
 ...
 *    All text is tailored to the teacher role; the primary CTA leads directly
 *    to the teacher dashboard (/[locale]/teacher/dashboard) rather than bare /.
 *
...
import { LayoutDashboard, ArrowLeft, GraduationCap } from 'lucide-react';
...
> = {
  uz: {
    ...
    body: "Siz qidirgan o'qituvchi sahifasi mavjud emas yoki ko'chirilgan. Dashboard'ga qaytib, kerakli bo'limni toping.",
    toDashboard: 'Dashboard\'ga qaytish',
    goBack: 'Orqaga',
    errorBadge: '404 — Sahifa topilmadi',
    goBackAriaLabel: 'Oldingi sahifaga qaytish',
    hint: "O'qituvchi bo'limi sahifasi",
  },
  en: {
    heading: 'Page Not Found',
    body: "The teacher page you're looking for doesn't exist or has been moved. Head back to your dashboard to find what you need.",
    toDashboard: 'Back to Dashboard',
    goBack: 'Go Back',
    errorBadge: '404 — Not Found',
    goBackAriaLabel: 'Go back to the previous page',
    hint: 'Teacher section page',
  },
  ru: {
    heading: 'Страница не найдена',
    body: 'Страница преподавателя, которую вы ищете, не существует или была перемещена. Вернитесь на панель управления.',
    toDashboard: 'Вернуться на панель',
    goBack: 'Назад',
    errorBadge: '404 — Не найдено',
    goBackAriaLabel: 'Перейти на предыдущую страницу',
    hint: 'Страница раздела преподавателя',
  },
};
...
export default function TeacherNotFound() {
  ...
  const dashboardHref = `/${locale}/teacher/dashboard`;

  // ── Render ─────────────────────────────────────────────────────────────────
  return (
    <div
      role="main"
      aria-labelledby="teacher-not-found-title"
      style={{
        display: 'flex',
        alignItems: 'center',
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <GraduationCap size={11} aria-hidden="true" strokeWidth={2.5} />
            {t.hint}
          </span>
        ...
        >
          {/* Primary: back to teacher dashboard */}
          <motion.div
        ...
        {process.env.NODE_ENV === 'development' && (
          ...
          >
            locale: {locale} · section: teacher · href: {dashboardHref}
          </p>
        )}
      </motion.div>
    </div>
