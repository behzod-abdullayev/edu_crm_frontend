 ...
 *
 * Teacher section layout — Server Component.
 *
 * Responsibilities:
 *  - Enforce role-based access: 'teacher', 'admin', and 'owner' may access teacher routes
 *    (admins and owners need visibility into teacher views for management purposes)
...
// Teacher routes accessible to: teacher (own), admin (supervision), owner (full access)
const ALLOWED_ROLES = ['teacher', 'admin', 'owner'] as const satisfies readonly UserRole[];
type AllowedTeacherRole = (typeof ALLOWED_ROLES)[number];

function isAllowedRole(role: UserRole): role is AllowedTeacherRole {
  return (ALLOWED_ROLES as readonly UserRole[]).includes(role);
}
...
export const metadata: Metadata = {
  ...
  description:
    'EduCRM teacher panel — manage groups, attendance, homework, lessons and student analytics.',
  robots: {
    index: false,
    follow: false,
    noarchive: true,
